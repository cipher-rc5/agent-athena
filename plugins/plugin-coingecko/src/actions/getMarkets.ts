...
import { type Action, type ActionExample, composeContext, type Content, elizaLogger, generateObject, type HandlerCallback, type IAgentRuntime, type Memory, ModelClass, type State } from '@elizaos/core';
import axios from 'axios';
import { z } from 'zod';
import { getApiConfig, validateCoingeckoConfig } from '../environment.ts';
import { getCategoriesData } from '../providers/categoriesProvider.ts';
import { getMarketsTemplate } from '../templates/markets.ts';
...
export function formatCategory(category: string | undefined, categories: CategoryItem[]): string | undefined {
  ...
  const nameMatch = categories.find(c =>
    c.name.toLowerCase() === normalizedInput || c.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === normalizedInput
  );
  if (nameMatch) {
  ...
  const partialMatch = categories.find(c =>
    c.name.toLowerCase().includes(normalizedInput) || c.category_id.includes(normalizedInput)
  );
  if (partialMatch) {
  ...
  handler: async (
    ...
    try {
      ...
      const result = await generateObject({
        ...
        schema: GetMarketsSchema as any
      });
      ...
      const response = await axios.get<CoinMarketData[]>(`${baseUrl}/coins/markets`, {
      ...
      const categoryDisplay = content.category ?
        `${
          categories.find(c => c.category_id === formattedCategory)?.name.toUpperCase() ||
          content.category.toUpperCase()
        } ` :
        '';

      const responseText = [
        `Top ${formattedData.length} ${categoryDisplay}Cryptocurrencies by ${
          content.order === 'volume_desc' || content.order === 'volume_asc' ? 'Volume' : 'Market Cap'
        }:`,
