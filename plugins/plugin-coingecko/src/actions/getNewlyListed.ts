...
import { getNewCoinsTemplate } from '../templates/newCoins.ts';
...
export const GetNewCoinsSchema = z.object({ limit: z.number().min(1).max(50).default(10) });
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const newCoinsContext = composeContext({ state, template: getNewCoinsTemplate });

      const result = await generateObject({
        runtime,
        context: newCoinsContext,
        modelClass: ModelClass.LARGE,
        schema: GetNewCoinsSchema as any
      });
      ...
      const response = await axios.get<NewCoinsResponse>(`${baseUrl}/coins/list/new`, {
      ...
      if (callback) {
        callback({ text: responseText, content: { newCoins: formattedData, timestamp: new Date().toISOString() } });
      }
