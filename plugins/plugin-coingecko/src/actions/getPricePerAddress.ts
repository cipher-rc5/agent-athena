...
import { getPriceByAddressTemplate } from '../templates/priceAddress.ts';

// Schema definition for the token price request
export const GetTokenPriceSchema = z.object({ chainId: z.string(), tokenAddress: z.string() });

export type GetTokenPriceContent = z.infer<typeof GetTokenPriceSchema> & Content;

export const isGetTokenPriceContent = (obj: any): obj is GetTokenPriceContent => {
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const context = composeContext({ state, template: getPriceByAddressTemplate });
      ...
      const result = await generateObject({
        ...
        schema: GetTokenPriceSchema as any
      });
      ...
      elizaLogger.error('Error in GET_TOKEN_PRICE_BY_ADDRESS handler:', error);
