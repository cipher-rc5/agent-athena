...
import { getTrendingTemplate } from '../templates/trending.ts';
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const trendingContext = composeContext({ state, template: getTrendingTemplate });
      ...
      const response = await axios.get<TrendingResponse>(`${baseUrl}/search/trending`, {
      ...
      if (callback) {
        callback({ text: responseText, content: { trending: formattedData, timestamp: new Date().toISOString() } });
      }
