...
import { getCoinsData } from '../providers/coinsProvider.ts';
import { getPriceTemplate } from '../templates/price.ts';
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const priceContext = composeContext({ state, template: getPriceTemplate });
      ...
      const response = await axios.get<PriceResponse>(`${baseUrl}/simple/price`, {
        params: {
          ids: coinIds,
          vs_currencies,
          include_market_cap: content.include_market_cap,
      ...
      const formattedResponse = Object.entries(response.data).map(([coinId, data]) => {
        ...
        currencies.forEach(currency => {
          ...
          if (data[currency]) {
            parts.push(
              `  ${upperCurrency}: ${
                data[currency].toLocaleString(undefined, { style: 'currency', currency: currency })
              }`
            );
          }
          ...
          if (content.include_24hr_vol) {
            ...
            if (volume !== undefined) {
              parts.push(
                `  24h Volume (${upperCurrency}): ${
                  volume.toLocaleString(undefined, { style: 'currency', currency: currency, maximumFractionDigits: 0 })
                }`
              );
            }
          }
