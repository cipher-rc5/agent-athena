...
export const priceFeedsSchema = z.object({
  ...
  data: z.object({
    feeds: z.array(z.object({ id: z.string(), attributes: priceFeedAttributesSchema })),
    count: z.number(),
...
export async function validatePriceFeedsData(content: unknown): Promise<boolean> {
  try {
    ...
    throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Price feeds validation failed', ErrorSeverity.HIGH, {
      error
    });
  }
}
