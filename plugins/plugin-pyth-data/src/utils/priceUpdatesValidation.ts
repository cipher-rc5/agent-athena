...
import { ErrorSeverity, PythErrorCode } from '../error/base.ts';
import { DataError } from '../error/data.ts';
...
export const priceUpdatesSchema = {
  ...
  properties: {
    ...
    options: {
      type: 'object',
      properties: {
        encoding: { type: 'string', enum: ['hex', 'base64'], description: 'Encoding format for the price updates' },
        parsed: { type: 'boolean', description: 'Whether to return parsed price updates' }
      }
    }
  }
};
...
export async function validatePriceUpdatesData(data: unknown): Promise<boolean> {
  try {
    ...
    if (!valid) {
      ...
      elizaLogger.error('Price updates validation failed', { errors, data });

      throw new DataError(PythErrorCode.DATA_VALIDATION_FAILED, 'Price updates validation failed', ErrorSeverity.HIGH, {
        errors: errors.map((err: ErrorObject) => ({ path: err.schemaPath, message: err.message, params: err.params })),
        data
      });
    }
    ...
    throw new DataError(PythErrorCode.DATA_SCHEMA_ERROR, 'Price updates validation error', ErrorSeverity.HIGH, {
      error: error instanceof Error ? error.message : String(error),
      data
    });
  }
