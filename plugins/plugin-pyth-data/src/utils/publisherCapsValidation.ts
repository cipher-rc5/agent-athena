...
export async function validatePublisherCapsData(data: unknown): Promise<boolean> {
  try {
    ...
    if (!valid) {
      ...
      elizaLogger.error('Publisher caps validation failed', { errors, data });
    ...
    throw new DataError(PythErrorCode.DATA_SCHEMA_ERROR, 'Publisher caps validation error', ErrorSeverity.HIGH, {
      error: error instanceof Error ? error.message : String(error),
      data
    });
  }
