...
const priceDataSchema = z.object({ price: z.string(), conf: z.string(), expo: z.number(), publish_time: z.number() });
...
const binaryDataSchema = z.object({ encoding: z.string(), data: z.array(z.string()) });
...
export async function validatePriceUpdateStreamData(content: unknown): Promise<boolean> {
  try {
    ...
    throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Price update stream validation failed', ErrorSeverity.HIGH, {
      error
    });
  }
}
