import { type ErrorDetails, ErrorSeverity, PythError, type PythErrorCode } from './base.ts';
...
type ValidationError = string | { field: string, message: string, value?: unknown };
...
export class DataError extends PythError {
  ...
  static invalidSubscription(subscription: unknown, reason: string): DataError {
    return new DataError(DataErrorCode.INVALID_SUBSCRIPTION, 'Invalid subscription parameters', ErrorSeverity.MEDIUM, {
      subscription,
      reason
    });
  }

  static subscriptionLimitExceeded(currentCount: number, maxLimit: number): DataError {
  ...
  static validationFailed(data: unknown, validationErrors: ValidationError[]): DataError {
    return new DataError(DataErrorCode.VALIDATION_FAILED, 'Data validation failed', ErrorSeverity.MEDIUM, {
      data,
      validationErrors
    });
  }

  static sequenceError(expected: number, received: number, details?: ErrorDetails): DataError {
    return new DataError(
  ...
  static transformError(data: unknown, targetFormat: string, error?: Error): DataError {
    return new DataError(DataErrorCode.TRANSFORM_ERROR, 'Failed to transform data', ErrorSeverity.MEDIUM, {
      data,
      targetFormat
    }, error);
  }
  ...
  static stalePriceError(symbol: string, lastUpdateTime: number, maxAge: number): DataError {
    return new DataError(DataErrorCode.STALE_PRICE, `Price data for ${symbol} is stale`, ErrorSeverity.HIGH, {
      symbol,
      lastUpdateTime,
      maxAge,
  ...
  static chainError(chain: string, operation: string, error?: Error): DataError {
    return new DataError(
