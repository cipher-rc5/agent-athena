import { type ErrorDetails, ErrorSeverity, PythError, PythErrorCode } from './base.ts';
...
export class WebSocketError extends PythError {
  ...
  static invalidUrl(url: string): WebSocketError {
    return new WebSocketError(PythErrorCode.WS_INVALID_URL, `Invalid WebSocket URL: ${url}`, ErrorSeverity.HIGH, {
      url
    });
  }

  static sslError(details?: ErrorDetails): WebSocketError {
    return new WebSocketError(PythErrorCode.WS_SSL_ERROR, 'SSL/TLS connection error', ErrorSeverity.HIGH, details);
  }
