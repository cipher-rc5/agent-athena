...
const AssetType = z.enum(['crypto', 'fx', 'equity', 'metal', 'rates', 'crypto_redemption_rate']);
const asset_type = AssetType.nullish();
const RpcPriceIdentifier = z.string();
const PriceFeedMetadata = z.object({ attributes: z.record(z.string()), id: RpcPriceIdentifier }).passthrough();
const PriceIdInput = z.string();
const EncodingType = z.enum(['hex', 'base64']);
const BinaryUpdate = z.object({ data: z.array(z.string()), encoding: EncodingType }).passthrough();
...
const PriceUpdate = z.object({ binary: BinaryUpdate, parsed: z.array(ParsedPriceUpdate).nullish() }).passthrough();
const ParsedPublisherStakeCap = z.object({ cap: z.number().int().gte(0), publisher: z.string() }).passthrough();
const ParsedPublisherStakeCapsUpdate = z.object({ publisher_stake_caps: z.array(ParsedPublisherStakeCap) })
  .passthrough();
...
const TwapsResponse = z.object({ binary: BinaryUpdate, parsed: z.array(ParsedPriceFeedTwap).nullish() }).passthrough();
...
and query string.`,
  requestFormat: 'json',
  parameters: [{ name: 'query', type: 'Query', schema: z.string().nullish() }, {
    name: 'asset_type',
    type: 'Query',
    schema: asset_type
  }],
...
Given a collection of price feed ids, retrieve the latest Pyth price for each price feed.`,
  requestFormat: 'json',
  parameters: [
    { name: 'publish_time', type: 'Path', schema: z.number().int() },
    { name: 'ids[]', type: 'Query', schema: z.array(PriceIdInput) },
    { name: 'encoding', type: 'Query', schema: z.enum(['hex', 'base64']).optional() },
    { name: 'parsed', type: 'Query', schema: z.boolean().optional() },
    { name: 'ignore_invalid_price_ids', type: 'Query', schema: z.boolean().optional() }
  ],
  response: PriceUpdate,
  errors: [{ status: 404, description: `Price ids not found`, schema: z.void() }]
}, {
  method: 'get',
...
Given a collection of price feed ids, retrieve the latest Pyth price for each price feed.`,
  ...
  parameters: [
    ...
    { name: 'encoding', type: 'Query', schema: z.enum(['hex', 'base64']).optional() },
    { name: 'parsed', type: 'Query', schema: z.boolean().optional() },
    { name: 'allow_unordered', type: 'Query', schema: z.boolean().optional() },
    { name: 'benchmarks_only', type: 'Query', schema: z.boolean().optional() },
    { name: 'ignore_invalid_price_ids', type: 'Query', schema: z.boolean().optional() }
  ],
  response: PriceUpdate,
  errors: [{ status: 404, description: `Price ids not found`, schema: z.void() }]
  ...
  requestFormat: 'json',
  parameters: [{ name: 'encoding', type: 'Query', schema: z.enum(['hex', 'base64']).optional() }, {
    name: 'parsed',
    type: 'Query',
...
Given a collection of price feed ids, retrieve the latest Pyth TWAP price for each price feed.`,
  requestFormat: 'json',
  parameters: [
    { name: 'window_seconds', type: 'Path', schema: z.number().int().gte(0) },
    { name: 'ids[]', type: 'Query', schema: z.array(PriceIdInput) },
    { name: 'encoding', type: 'Query', schema: z.enum(['hex', 'base64']).optional() },
    { name: 'parsed', type: 'Query', schema: z.boolean().optional() },
    { name: 'ignore_invalid_price_ids', type: 'Query', schema: z.boolean().optional() }
