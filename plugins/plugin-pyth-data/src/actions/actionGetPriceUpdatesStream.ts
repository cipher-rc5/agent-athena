...
import { validatePriceUpdateStreamData } from '../utils/priceUpdateStreamValidation.ts';
...
interface GetPriceUpdatesStreamContent extends Content {
  text: string;
  priceIds: string[];
  options?: { encoding?: 'hex' | 'base64', parsed?: boolean, allowUnordered?: boolean, benchmarksOnly?: boolean };
  success?: boolean;
  data?: {
    streamId: string,
...
async function collectStreamData(eventSource: EventSource, streamId: string): Promise<RawPriceData> {
  return new Promise((resolve, reject) => {
    let messageCount = 0;
    const collectedData: RawPriceData = { binary: { encoding: 'hex', data: [] }, parsed: [] };
...
async function createPriceStream(
  ...
  try {
    const eventSource =
      (await client.getPriceUpdatesStream(priceIds, { parsed: true, encoding: 'hex' })) as unknown as EventSource;
        ...
        callback(
          {
            ...
            success: true,
            priceIds,
            data: { streamId, status: 'connected', ...processedData }
        ...
        callback(
          {
            ...
            success: false,
            priceIds,
            data: { streamId, status: 'error', error: error instanceof Error ? error.message : String(error) }
          } as GetPriceUpdatesStreamContent
        );
      }
      activeStreams.delete(streamId);
...
export const getPriceUpdatesStreamAction: Action = {
  ...
  examples: [[
    {
      ...
      content: {
        ...
        options: { encoding: 'hex', parsed: true, benchmarksOnly: true }
      } as GetPriceUpdatesStreamContent
    } as ActionExample,
    {
      user: 'assistant',
  ...
  ], [
    {
      ...
      content: {
        ...
        options: { benchmarksOnly: true, parsed: true }
      } as GetPriceUpdatesStreamContent
    } as ActionExample,
    {
      user: 'assistant',
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    logGranular('Starting validation', { content: message.content });
    ...
    try {
      ...
      if (!content.priceIds) {
        ...
        if (priceIds.length > 0) {
          content = { ...content, priceIds, options: { parsed: true, encoding: 'hex' } };
  ...
  async handler(
    ...
    logGranular('Starting handler execution', { messageId: message.id, messageContent: message.content });
