...
import { getConfig, getNetworkConfig, validatePythConfig } from '../environment.ts';
import { ErrorSeverity } from '../error/base.ts';
import { DataError, DataErrorCode } from '../error/data.ts';
import { validatePriceFeedsData } from '../utils/priceFeedsValidation.ts';
...
interface GetPriceFeedsContent extends Content {
  ...
  success?: boolean;
  data?: {
    feeds: Array<{ id: string, attributes: PriceFeedAttributes }>,
    count: number,
    responseType: string,
    isArray: boolean,
    error?: string
...
export const getPriceFeedsAction: Action = {
  ...
  async validate(runtime: IAgentRuntime, message: Memory): Promise<boolean> {
    ...
    logGranular('Starting validation', { messageId: message.id, content: message.content });
    ...
    try {
      ...
      try {
        ...
        if (error instanceof DataError) {
          elizaLogger.error('Schema validation failed', { errors: error.details?.errors });
          throw error;
        }
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Schema validation failed', ErrorSeverity.HIGH, { error });
      }
      ...
      if (!config) {
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Invalid Pyth configuration', ErrorSeverity.HIGH);
      }
      ...
      if (content.query && typeof content.query !== 'string') {
        ...
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Query must be a string', ErrorSeverity.HIGH);
      }

      if (content.filter && typeof content.filter !== 'string') {
        logGranular('Invalid filter type', { filter: content.filter });
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Filter must be a string', ErrorSeverity.HIGH);
      }

      logGranular('Validation successful', { query: content.query, filter: content.filter });
      ...
      throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Invalid content format', ErrorSeverity.HIGH, {
  ...
  async handler(
    ...
    logGranular('Starting handler execution', { messageId: message.id, hasCallback: !!callback });
    ...
    try {
      ...
      logGranular('Initialized HermesClient', { endpoint: networkConfig.hermes });

      // Get price feeds with proper options
      const options = { query: query, filter: filter };
...
${query ? `\nMatching query: "${query}"` : ''}${filter ? `\nWith filter: "${filter}"` : ''}`,
        ...
        filter,
        success: true,
        data: { feeds: transformedFeeds, count: priceFeeds.length, responseType: 'object', isArray: true }
      };

      logGranular('Prepared callback content', { feedCount: transformedFeeds.length, firstFeed: transformedFeeds[0] });
      ...
      logGranular('Error retrieving price feeds', { error: errorMessage, stack: errorStack });
      ...
      const errorContent: GetPriceFeedsContent = {
        ...
        success: false,
        data: { feeds: [], error: errorMessage, count: 0, responseType: 'object', isArray: true }
      };
      ...
      throw new DataError(DataErrorCode.PRICE_FEEDS_FETCH_FAILED, errorMessage, ErrorSeverity.HIGH);
    }
  }
};
