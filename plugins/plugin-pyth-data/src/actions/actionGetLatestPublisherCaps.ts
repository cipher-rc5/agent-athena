...
import { validatePublisherCapsData } from '../utils/publisherCapsValidation.ts';
...
interface GetLatestPublisherCapsContent extends Content {
  text: string;
  success?: boolean;
  data?: { caps?: Array<{ publisher: string, cap: number, timestamp: number }>, error?: string };
}
...
export const getLatestPublisherCapsAction: Action = {
  ...
  examples: [[
    {
      user: 'user',
      content: { text: 'Get me all the latest publisher caps' } as GetLatestPublisherCapsContent
    } as ActionExample,
    {
      user: 'assistant',
      content: {
        ...
        success: true,
        data: {
          caps: [{ publisher: '0x1234567890abcdef1234567890abcdef12345678', cap: 1000000, timestamp: 1641034800 }]
        }
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    logGranular('Validating GET_LATEST_PUBLISHER_CAPS action', { content: message.content });
    ...
    try {
      ...
      try {
        ...
        if (error instanceof DataError) {
          elizaLogger.error('Publisher caps validation failed', { errors: error.details?.errors });
          throw error;
        }
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'Publisher caps validation failed', ErrorSeverity.HIGH, {
          error
        });
      }
  ...
  handler: async (
    ...
    try {
      ...
      try {
        ...
        const response = await hermesClient.getLatestPublisherCaps({ parsed: true });
        ...
        const formattedText = publisherCaps.map((cap, index) => `Publisher ${index + 1}:
ID: ${cap.publisher}
Cap: ${cap.cap.toLocaleString()} tokens
Timestamp: ${new Date(currentTimestamp).toLocaleString()}`).join('\n\n');
      ...
      throw new DataError(DataErrorCode.NETWORK_ERROR, 'Failed to get publisher caps', ErrorSeverity.HIGH, {
        originalError: error
      });
    }
  }
