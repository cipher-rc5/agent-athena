...
import { validatePriceUpdatesData } from '../utils/priceUpdatesValidation.ts';
    ...
    updates?: Array<
      {
        ...
        expo: number,
        publish_time: number,
        ema_price?: { price: number, conf: number, expo: number }
      }
    >,
    error?: string
  };
...
export const getLatestPriceUpdatesAction: Action = {
  ...
  examples: [[
    {
      ...
      content: {
        ...
        options: { encoding: 'base64', parsed: true }
      ...
      content: {
        ...
        data: {
          updates: [{
            ...
            ema_price: { price: 41950000000, conf: 95000000, expo: -8 }
          }]
        }
      } as GetLatestPriceUpdatesContent
    } as ActionExample
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ...
    logGranular('Validating GET_LATEST_PRICE_UPDATES action', { content: message.content });
    ...
    try {
      ...
      if (content.priceIds.length === 0) {
        throw new DataError(DataErrorCode.VALIDATION_FAILED, 'priceIds array cannot be empty', ErrorSeverity.HIGH);
      }
  ...
  handler: async (
    ...
    try {
      ...
      logGranular('Extracted values', { priceIds, options });
      ...
      try {
        ...
        logGranular('Requesting price updates with params', { priceIds, options });
        ...
        logGranular('Raw response from Hermes', { updates });

        if (!updates || !updates.parsed) {
          throw new Error('No updates received from Hermes');
        }

        logGranular('Successfully retrieved price updates', { updates, parsedCount: updates.parsed?.length });
...
Proof Available: ${proofTime ? new Date(proofTime * 1000).toLocaleString() : 'Not available'}` :
      ...
      throw new DataError(DataErrorCode.NETWORK_ERROR, 'Failed to get latest price updates', ErrorSeverity.HIGH, {
        originalError: error
      });
    }
  }
