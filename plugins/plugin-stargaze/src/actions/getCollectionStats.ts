...
import { type Action, composeContext, type Content, elizaLogger, generateObjectDeprecated, type HandlerCallback, type IAgentRuntime, type Memory, ModelClass, type State } from '@elizaos/core';
import axios from 'axios';
import { validateStargazeConfig } from '../environment.ts';
...
export default {
  ...
  handler: async (
    ...
    try {
      ...
      const formatPercent = (value: number) => value ? `${Number(value).toFixed(2)}%` : '0%';
...
- Market Cap: ${formatValue(stats.marketCap)} USD`,
      ...
      debugLog.error(error);
      if (callback) {
        callback({ text: `Error fetching collection stats: ${error}`, content: { error: error } });
      }
      return false;
    }
  },
